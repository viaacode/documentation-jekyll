...
<mark>to do: veel te veel te veel secties?</mark>

<mark>to do: gebruik van metadata terms/metadata elements/metadata tags?</mark>

<mark>to do: weglaten want automatisch gegenereerd op website?</mark>
...
<mark>to do: verder onderverdelen in subsecties rond typography/keywords?</mark>
...
> This section and the examples throughout this specification based upon it, are to be considered informative.
...
<mark>to do: verder uitschrijven of is dit uitgebreid genoeg voor de spec?</mark>
...
<mark>to do: verder uitschrijven of redundant met volgende sectie?</mark>
...
The */data* directory contains the content of the bag, i.e. the media file and its accompanying metadata files, divided across a number of different files and directories.
...
In the case of the meemoo SIP, the *mets.xml* file merely acts as an inventory of the files and directories.
...
It is therefore important to note that the package *mets.xml* file does not record the internal structure of the different representations in the */representations* directory.
...
The */descriptive* directory contains descriptive metadata about the (sub-)IE(s) at package level.
...
It relies on the [Preservation Metadata: Implementation Strategies (PREMIS) standard](https://www.loc.gov/standards/premis/) in order to provide basic preservation information.
...
The */representations* directory contains a separate */representation_n* directory (where n is a positive integer) for each representation of the (sub)IE(s) in the package level. 
...
The */data* directory contains the payload files of the SIP, also known as the media files or essence.

***Requirements***

- The */data* MUST NOT contain any subdirectories.
- All files in the */data* directory MUST be referenced in their respective representation *mets.xml* files.
